...
@router.post("/sign_up", status_code=status.HTTP_200_OK, response_model=TokenSchema)
def sign_up(user_to_create: UserCreate, db=Depends(get_db)):
...
@router.post("/sign_in", status_code=status.HTTP_200_OK, response_model=TokenSchema)
def sign_in(user_to_login: UserCreate, db=Depends(get_db)):
...
#     notif = NotifCreate(type=NotifType.ERROR, data="Error")
